...
calc <- function( grupo) {
  a <- svyby( formula = ~piso_tierra + agua_red_publica + desague_red_publica + electricidad + cocina_lena,
              ...
              row.names  = FALSE)
  a1 <- a %>% select( 1, !starts_with("cv.")) %>% pivot_longer( !1, names_to = "indicador", values_to = "resultado")
  a2 <- a %>% select( 1, starts_with("cv.")) %>% pivot_longer(  !1, names_to = "indicador", values_to = "resultado")
  b <- bind_rows( a1, a2) %>% 
    mutate( tipo = case_when( str_extract(indicador, "cv.") == "cv." ~ "cv", TRUE ~"resultado"),
                                      indicador = str_remove( indicador, "cv.")) %>% 
    pivot_wider( names_from = tipo, values_from = resultado)
}
```
...
comparacion
```
Se presenta un resumen de las diferencias entre resultados y coeficientes calculados con ambos métodos:
```{r resumen}
